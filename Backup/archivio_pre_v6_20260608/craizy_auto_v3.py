...
PORT = 3001
MAX_STEPS = 100000
DRIVER_VERSION = "craizy_auto_v3_launch_racing_profile"
...
MAX_STEER_CORRECTION = 0.08
MIN_STEER_CORRECTION = 0.06
...
RECOVERY_BLEND_IN = 0.025
RECOVERY_BLEND_IN_DANGER = 0.015
RECOVERY_BLEND_OUT = 0.04
RECOVERY_TARGET_SPEED = 70.0
RECOVERY_CONFIRM_TICKS = 5
RECOVERY_EXTREME_TRACK_POS = 1.05
...
class PostAdasReplayPolicy:
    ...
    def action(self, sensors):
        ...
        violations = []
        if abs(line_error) > enter_line:
            violations.append("line")
        if abs(angle_error) > enter_angle:
            violations.append("angle")
        if abs(lateral_error) > enter_lateral:
            violations.append("lateral")
        if abs(track_pos) > enter_track:
            violations.append("track")

        if abs(speed) < STUCK_SPEED:
            self.stuck_ticks += 1
        else:
            self.stuck_ticks = 0
        stuck = self.stuck_ticks >= STUCK_TICKS
        extreme_track = abs(track_pos) > RECOVERY_EXTREME_TRACK_POS
        extreme_angle = abs(safe_float(sensors.get("angle"))) > (
            RECOVERY_EXTREME_ANGLE
        )
        ...
        if normal_recovery_allowed and violations:
        ...
        elif self.recovery_confirmation_ticks >= RECOVERY_CONFIRM_TICKS:
            recovery_cause = "+".join(violations)
...
class TraceLogger:
    FIELDS = [
        ...
        "recovery_blend", "recovery_confirmation_ticks", "recovery_cause",
    ...
    def __init__(self, path=None):
        if path is None:
            ...
            path = os.path.join(
                LOGS_DIR,
                "auto_v3_trace_%s.csv" % timestamp,
            )
        self.path = path
